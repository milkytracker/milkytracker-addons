...
<CsInstruments>
; this Synth is an extended version of Ian McCurdy's awesome subtractive example
; from the csound flossmanual https://flossmanual.csound.com/sound-synthesis/subtractive-synthesis
;
; usage: csound --smacro:length=1 --smacro:note=50 --strset1=/tmp/out.wav ianmcurdy_substractive.csd
...
gSout strget 1                 ; milkytracker output file
...
;i  1  0  1  50   .2  2  .5   0  -5  .2  2   0.5 0    5   1   .01  1    .1   .1  .5  .005 .01 1   .05
...
i 1 + 1  $note $osc1amp $osc1type $osc1PWM $osc1oct $osc1cent $osc2amp $osc2type $osc2PWM $osc1oct  $osc1cent  $cutoff  $fattack $fdecay  $fsustain  $frelease $resonance $attack $decay $sustain $release
